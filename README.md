...
<p align="center">
  Превращает фото в оригинальный <b>16-битный chiptune</b> прямо на телефоне.<br/>
...
<p align="center">
  <img src="docs/capture.webp" width="45%" alt="Экран съёмки">
  <img src="docs/player.webp" width="45%" alt="Плеер">
</p>

<p align="center"><sub><a href="https://github.com/alex90132/chipmood/raw/main/docs/demo.mp4">Демо со звуком</a></sub></p>

## Что умеет

ChipMood сочиняет музыку прямо на устройстве по фотографии. Наводишь камеру на
что угодно, жмёшь один раз. Приложение определяет настроение по цветам снимка,
собирает целый трек из библиотеки реальных игровых фраз и играет его на
собственном чип-синтезаторе (импульс, треугольник, пила, шум). Задача: чтобы
звучало так, будто написал и сыграл живой человек, но с сырой «чиповой душой»
классического chiptune.

- **Фото в музыку одним нажатием.** Лёгкий анализ цвета (яркость, теплота,
...
- **Полностью офлайн.** Без облака, аккаунта и кредитов. Весь звук даёт
  собственный синтезатор на Rust, сэмплы не используются.
- **Ремикс через любой ИИ-чат.** Кнопка **Copy** собирает компактный промпт с
  реальными «хит-фразами». Вставляешь ответ обратно кнопкой **Paste**, и трек
...
1. **Фото в настроение.** Анализ цвета через `dart:ui` раскладывает снимок по
...
7. **Мастеринг и вывод.** Glue-компрессор, левелер, лимитер и лёгкий реверб.
...
- **NES-MDB** и General-MIDI (мульти-трек). Chiptune и широкая мелодика.
- **POP909.** Реальные поп-прогрессии аккордов и контрапункт.
...
`ml/model.py`, `ml/generate.py`, чек-пойнт `ckpt.pt`). Вышло плохо: бессвязный,
бесструктурный вывод. Для вменяемой модели символьной музыки нужно несравнимо
больше данных, вычислений и времени. Поэтому перешли к подходу
retrieval-augmented плюс правила, и звучит он несравнимо лучше. Нейропуть остался
в коде опционально, но не по умолчанию.
...
Звук ChipMood идёт от собственного синтезатора, сэмплы не используются.
Библиотека референсов содержит только нормализованные, преобразованные ноты.
Защищённый авторским правом исходный материал (модули Unreal/UT99, сырые
датасеты, чек-пойнты) не включён и лежит в `.gitignore`. Чтобы перезапустить
майнеры, принеси свои копии.
...
Public domain, лицензия **The Unlicense** (см. [`LICENSE`](LICENSE)). Делай что
угодно. Сторонние датасеты, использованные для генерации данных, имеют свои
условия.
